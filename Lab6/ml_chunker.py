...
def predict(test_sentences, feature_names, f_out):
    ...
    for test_sentence in test_sentences:
        ...
        for j in range(len(X_test_dict)):
            X_test_dict[j]['tag_n2'] = predicted2
            X_test_dict[j]['tag_n1'] = predicted1

            # Vectorize the test sentence and one hot encoding
            X_test = vec.transform(X_test_dict[j])

            # Predicts the chunks and returns numbers
            temp = classifier.predict(X_test)

            y_test_predicted.append(temp[0])


       # print(y_test_predicted)
            predicted2 = predicted1
            predicted1 = y_test_predicted[j]
...
if __name__ == '__main__':
    ...
    no_tag_corpus = 'nochunk.txt'
    w_size = 2  # The size of the context window to the left and right of the word
    feature_names = ['word_n2', 'word_n1', 'word', 'word_p1', 'word_p2',
                     'pos_n2', 'pos_n1', 'pos', 'pos_p1', 'pos_p2','tag_n2','tag_n1']
   # feature_names = ['word_n2', 'word_n1', 'word', 'word_p1', 'word_p2']
